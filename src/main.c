...
static void print_usage(FILE *fp)
{
	fprintf(fp,
	        "piduier %s\n"
	        "PiDuier is a Raspberry Pi 5 40-pin interface debugging platform "
	        "for embedded software and hardware development.\n"
	        "Usage: piduier [options]\n"
	        "  -f, --config PATH   application config (JSON), default: "
	        "./piduier.conf\n"
...
int main(int argc, char **argv)
{
	...
	LOG_INFO(
	    "piduier starting product=\"Raspberry Pi 5 40-pin interface debugging "
	    "platform\" version=%s arch=%s config=%s http_listen=%s http_port=%d "
	    "log_file=%s",
	    PIDUIER_VERSION, PIDUIER_BUILD_ARCH, config_path, g_cfg.http_listen,
	    g_cfg.http_port, g_cfg.log_file);

	if (gpio_apply_pinctrl_config(g_cfg.pinctrl, PIDUIER_PINCTRL_GPIO_COUNT) !=
	    0) {
	...
	if (snprintf(listen_url, sizeof(listen_url), "http://%s:%d",
	             g_cfg.http_listen,
	             g_cfg.http_port) >= (int)sizeof(listen_url)) {
