...
static void parse_timedatectl_line(const char *line, datetime_info_t *info) {
    if (strstr(line, "Local time:") != NULL) {
        ...
        if (colon) {
            ...
            while (*date_start != ' ' && *date_start != '\0') date_start++;
            while (*date_start == ' ') date_start++;
            if (sscanf(date_start, "%10s %8s", info->local_time, info->local_time + 11) == 2) {
                info->local_time[10] = ' ';
        ...
        if (colon) {
            ...
            while (*date_start != ' ' && *date_start != '\0') date_start++;
            while (*date_start == ' ') date_start++;
            if (sscanf(date_start, "%10s %8s", info->utc_time, info->utc_time + 11) == 2) {
                info->utc_time[10] = ' ';
                info->utc_time[19] = '\0';
            }
        }
        ...
        if (!offset_start) offset_start = strchr(line, '-');
        if (offset_start) {
...
int datetime_set_time(const char *datetime) {
    ...
    char cmd[512];
    snprintf(cmd, sizeof(cmd), "timedatectl set-time \"%s\" 2>/dev/null", datetime);
