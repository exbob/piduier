...
int system_get_info(system_info_t *info) {
    ...
    if (strlen(info->hardware_vendor) == 0 || strlen(info->hardware_model) == 0) {
        ...
        if (f != NULL) {
            ...
            if (fgets(model, sizeof(model), f) != NULL) {
                ...
                if (strlen(info->hardware_model) == 0) {
                    snprintf(info->hardware_model, sizeof(info->hardware_model), "%s", model);
                }
                if (strlen(info->hardware_vendor) == 0) {
                    if (strstr(model, "Raspberry Pi") != NULL) {
                        snprintf(info->hardware_vendor, sizeof(info->hardware_vendor), "Raspberry Pi Foundation");
                    }
                }
            }
            fclose(f);
...
int system_set_hostname(const char *hostname) {
    ...
    char cmd[512];
    snprintf(cmd, sizeof(cmd), "hostnamectl set-hostname %s 2>/dev/null", hostname);
...
int system_set_pretty_hostname(const char *pretty_hostname) {
    ...
    char cmd[512];
    snprintf(cmd, sizeof(cmd), "hostnamectl set-hostname --pretty \"%s\" 2>/dev/null", pretty_hostname);
    int result = system(cmd);
    return (result == 0) ? 0 : -1;
}
