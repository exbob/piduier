...
static int check_object_all_strings(const cJSON *obj, const char *ctx,
    ...
    cJSON_ArrayForEach(ch, obj) {
        if (!cJSON_IsString(ch) || ch->valuestring == NULL) {
            snprintf(errbuf, errbuf_sz, "%s: value for key \"%s\" must be a string",
                     ctx, ch->string ? ch->string : "?");
            return -1;
        }
    }
    return 0;
...
int piduier_config_load(const char *path, piduier_config_t *cfg,
    ...
    if (!cJSON_IsString(j_log_file) || j_log_file->valuestring == NULL) {
        set_err(errbuf, errbuf_sz, "zlog.log_file is required and must be a string");
        goto done;
    }
    ...
    if (check_object_all_strings(global, "zlog.global", errbuf, errbuf_sz) != 0) {
        goto done;
    }

    cJSON *formats = cJSON_GetObjectItemCaseSensitive(zlog, "formats");
    if (check_object_all_strings(formats, "zlog.formats", errbuf, errbuf_sz) != 0) {
        goto done;
    }
    ...
    cJSON_ArrayForEach(r, rules) {
        if (!cJSON_IsString(r) || r->valuestring == NULL) {
            set_err(errbuf, errbuf_sz, "each zlog.rules[] entry must be a string");
            goto done;
        }
    }
    ...
    fprintf(out, "\n[rules]\n");
    cJSON_ArrayForEach(r, rules) {
        char *line = substitute_log_file(r->valuestring, cfg->log_file, errbuf, errbuf_sz);
        if (line == NULL) {
            fclose(out);
            free(mem);
            goto done;
