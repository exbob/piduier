...
static void handle_system_version(struct mg_connection *c,
		...
		if (hostname != NULL && strlen(hostname) > 0) {
			...
			if (result == 0) {
				...
				mg_http_reply(c, 400, "Content-Type: application/json\r\n",
				              ...
				              "hostname or failed to set\"}\n");
			}
			free(hostname);
		} else {
			mg_http_reply(c, 400, "Content-Type: application/json\r\n",
			              "{\"status\":\"error\",\"message\":\"Missing or "
		...
		if (timezones != NULL) {
			...
			if (json != NULL) {
				...
				for (size_t i = 0; i < count; i++) {
					int written =
					    snprintf(p, json_size - (p - json), "%s\"%s\"",
					             (i > 0) ? "," : "", timezones[i]);
					if (written > 0) {
						p += written;
					}
...
static void handle_temperature(struct mg_connection *c,
                               struct mg_http_message *hm)
{
	if (mg_match(hm->method, mg_str("GET"), NULL)) {
...
static void handle_network_devices(struct mg_connection *c,
                                   struct mg_http_message *hm)
{
	if (mg_match(hm->method, mg_str("GET"), NULL)) {
...
static void handle_gpio_status(struct mg_connection *c,
                               struct mg_http_message *hm)
{
	if (mg_match(hm->method, mg_str("GET"), NULL)) {
...
static int parse_gpio_num_from_uri(const struct mg_str uri)
{
	...
	if (sscanf(uri_buf, "/api/gpio/%d/mode", &gpio_num) == 1) {
		LOG_DEBUG("[GPIO][router] parse uri='%s' -> gpio=%d (mode)", uri_buf,
		          gpio_num);
		return gpio_num;
	}
	if (sscanf(uri_buf, "/api/gpio/%d/value", &gpio_num) == 1) {
		LOG_DEBUG("[GPIO][router] parse uri='%s' -> gpio=%d (value)", uri_buf,
		          gpio_num);
		return gpio_num;
	}
	if (sscanf(uri_buf, "/api/gpio/%d", &gpio_num) == 1) {
		LOG_DEBUG("[GPIO][router] parse uri='%s' -> gpio=%d (pin)", uri_buf,
		          gpio_num);
		return gpio_num;
	}
...
static void handle_gpio_mode(struct mg_connection *c,
                             struct mg_http_message *hm)
{
	if (mg_match(hm->method, mg_str("POST"), NULL)) {
		...
		if (gpio_num < 0 || !gpio_is_supported(gpio_num)) {
			LOG_INFO(
			    "[GPIO][router] mode request rejected: gpio=%d supported=%d",
		...
		if (mode != NULL) {
			if (strcmp(mode, "input") == 0) {
				...
				initial_value =
				    (int)mg_json_get_long(hm->body, "$.initial_value", 0);
		...
		if (result == 0) {
			if (mode != NULL && strcmp(mode, "input") == 0) {
				...
				LOG_INFO("[GPIO] gpio=%d configured as output initial=%d",
				         gpio_num, initial_value);
			} else if (mode != NULL) {
			...
			LOG_ERROR(
			    "[GPIO][router] mode set failed gpio=%d mode=%s result=%d "
			    "errno=%d",
...
static void handle_gpio_value(struct mg_connection *c,
                              struct mg_http_message *hm)
{
	if (mg_match(hm->method, mg_str("GET"), NULL)) {
		int gpio_num = parse_gpio_num_from_uri(hm->uri);
		...
		if (gpio_num < 0 || !gpio_is_supported(gpio_num)) {
			LOG_INFO(
			    "[GPIO][router] value request rejected: gpio=%d supported=%d",
			    gpio_num, gpio_is_supported(gpio_num));
			mg_http_reply(c, 400, "Content-Type: application/json\r\n",
			              "{\"error\":\"Invalid or unsupported GPIO pin\"}\n");
			return;
		...
		if (result == 0) {
			...
			LOG_ERROR(
			    "[GPIO][router] set value failed gpio=%d value=%d result=%d "
			    "errno=%d",
...
static void handle_gpio_attrs(struct mg_connection *c,
                              struct mg_http_message *hm)
{
	int gpio_num = parse_gpio_num_from_uri(hm->uri);
	if (gpio_num < 0 || !gpio_is_supported(gpio_num)) {
	...
	if (mg_match(hm->method, mg_str("GET"), NULL)) {
		...
		if (gpio_get_attrs(gpio_num, &attrs) == 0) {
			mg_http_reply(c, 200, "Content-Type: application/json\r\n",
			              "{\"gpio_num\":%d,\"pull\":\"%s\",\"drive\":\"%s\","
			              "\"available\":%d}\n",
			              attrs.gpio_num, pull_to_str(attrs.pull),
			              drive_to_str(attrs.drive), attrs.available);
		} else {
			mg_http_reply(
			    c, 503, "Content-Type: application/json\r\n",
	...
	if (mg_match(hm->method, mg_str("POST"), NULL)) {
		...
		if (!has_pull && !has_drive) {
			mg_http_reply(c, 400, "Content-Type: application/json\r\n",
			              "{\"error\":\"Nothing to update. Provide pull and/or "
			              "drive.\"}\n");
			if (pull)
				free(pull);
			if (drive)
		...
		LOG_INFO("[GPIO] gpio=%d attrs updated pull=%s drive=%s", gpio_num,
...
static void handle_system_info_complete(struct mg_connection *c,
                                        struct mg_http_message *hm)
{
	if (mg_match(hm->method, mg_str("GET"), NULL)) {
		// 构建完整的系统信息 JSON
		...
		int storage_ok =
		    (storage_get_info("/dev/mmcblk0p2", &storage_info) == 0);
		char *storage_json =
		    storage_ok ? storage_info_to_json(&storage_info) : NULL;
		...
		if (json != NULL) {
			int len = snprintf(
			    ...
			    "\"network\":%s"
			    "}",
			    sys_json ? sys_json : "null", dt_json ? dt_json : "null",
			    cpu_usage >= 0 ? cpu_usage : 0.0, mem_json ? mem_json : "null",
			    uptime_sec >= 0 ? uptime_sec : 0, uptime_str,
...
static void handle_reboot(struct mg_connection *c, struct mg_http_message *hm)
{
	if (mg_match(hm->method, mg_str("POST"), NULL)) {
		...
		if (result == 0) {
			...
			mg_http_reply(c, 200, "Content-Type: application/json\r\n",
			              "{\"status\":\"success\",\"message\":\"System reboot "
			              "initiated\"}\n");
		} else {
			mg_http_reply(c, 500, "Content-Type: application/json\r\n",
			              "{\"status\":\"error\",\"message\":\"Failed to "
		...
		mg_http_reply(
		    c, 405, "Content-Type: application/json\r\n",
...
void router_handle_request(struct mg_connection *c, struct mg_http_message *hm)
{
	...
	// 静态文件服务
		...
		struct mg_http_serve_opts opts = {.root_dir = g_web_root,
		                                  .fs = &mg_fs_posix};
...
void router_broadcast_gpio_if_changed(struct mg_mgr *mgr)
{
	...
	for (struct mg_connection *it = mgr->conns; it != NULL; it = it->next) {
		if (it->is_websocket && it->data[0] == 1 && !it->is_closing) {
			mg_ws_send(it, last_gpio_ws_json, strlen(last_gpio_ws_json),
			           WEBSOCKET_OP_TEXT);
		}
	}
}
