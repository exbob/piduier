...
static int parse_device_status(const char *line, network_device_t *device)
{
	...
	if (sscanf(line, "%15s %15s %31s %63[^\n]", device_name, device_type,
		...
		if (connection[0]) {
			snprintf(device->connection, sizeof(device->connection), "%s",
			         connection);
		}
		return 1;
	}
...
static void get_device_details(network_device_t *device)
{
	...
	snprintf(cmd, sizeof(cmd), "nmcli device show %s 2>/dev/null",
	         device->device);
	FILE *fp = popen(cmd, "r");
	if (fp == NULL) {
		return;
	...
	while (fgets(line, sizeof(line), fp) != NULL) {
		if (strstr(line, "IP4.ADDRESS[1]:")) {
			...
			if (strstr(line, "WIFI.SSID:") ||
			    strstr(line, "GENERAL.CONNECTION:")) {
				char *colon = strchr(line, ':');
				if (colon) {
					colon++;
					while (*colon == ' ' || *colon == '\t')
			...
			} else if (strstr(line, "WIFI.SIGNAL:") ||
			           strstr(line, "SIGNAL:")) {
				char *colon = strchr(line, ':');
				if (colon) {
					colon++;
	...
	if (strcmp(device->type, "ethernet") == 0 && strlen(device->speed) == 0) {
		char speed_file[256];
		snprintf(speed_file, sizeof(speed_file), "/sys/class/net/%s/speed",
		         device->device);
		FILE *f = fopen(speed_file, "r");
		if (f != NULL) {
			int speed_mbps;
			if (fscanf(f, "%d", &speed_mbps) == 1 && speed_mbps > 0) {
				snprintf(device->speed, sizeof(device->speed), "%d",
				         speed_mbps);
			}
			fclose(f);
		}
	...
	if (strcmp(device->type, "wifi") == 0) {
		snprintf(cmd, sizeof(cmd),
		         "nmcli device wifi list --rescan no 2>/dev/null | grep '^\\*' "
		         "| head -1");
		fp = popen(cmd, "r");
		if (fp) {
			if (fgets(line, sizeof(line), fp) != NULL) {
				// 解析格式：*  BSSID  SSID  MODE  CHAN  RATE  SIGNAL  BARS
				// SECURITY 使用 sscanf 按列解析
				char bssid[32] = {0};
				char ssid[64] = {0};
				char mode[16] = {0};
				char chan[16] = {0};
				...
				if (sscanf(p, "%31s %63s %15s %15s %31s %d %15s %31s", bssid,
				           ssid, mode, chan, rate, &signal, bars,
				           security) >= 6) {
					// 成功解析
					if (strlen(device->ssid) == 0 && strlen(ssid) > 0) {
						snprintf(device->ssid, sizeof(device->ssid), "%s",
						         ssid);
					}
					...
					if (num_start > line) {
						if (sscanf(num_start, "%d", &signal) == 1 &&
						    signal >= 0 && signal <= 100) {
							device->signal = signal;
						}
					}
	...
	while (fgets(line, sizeof(line), fp) != NULL) {
		...
		if (parse_device_status(line, &device) > 0) {
			...
			if (list->count >= capacity) {
				capacity *= 2;
				network_device_t *new_devices =
				    realloc(list->devices, capacity * sizeof(network_device_t));
				if (new_devices == NULL) {
					pclose(fp);
					network_device_list_free(list);
					return -1;
...
char *network_device_list_to_json(const network_device_list_t *list)
{
	...
	for (size_t i = 0; i < list->count; i++) {
		...
		int written = snprintf(p, size - (p - json),
		                       ...
		                       "\"signal\":%d"
		                       "}",
		                       (i > 0) ? "," : "", dev->device, dev->type,
		                       dev->state, dev->connection, dev->ip, dev->mac,
		                       dev->speed, dev->ssid, dev->signal);
