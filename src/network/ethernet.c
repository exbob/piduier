...
static int find_connection_name(const char *device, char *connection_name,
                                size_t size)
{
	char cmd[256];
	snprintf(cmd, sizeof(cmd),
	         "nmcli -t -f NAME,DEVICE connection show | grep ':%s$' | head -1 "
	         "| cut -d: -f1",
	         device);
...
int ethernet_get_config(const char *device, ethernet_config_t *config)
{
	...
	if (find_connection_name(device, config->connection_name,
	                         sizeof(config->connection_name)) != 0) {
		snprintf(config->connection_name, sizeof(config->connection_name),
		         "Wired connection 1");
	}

	char cmd[512];
	snprintf(cmd, sizeof(cmd), "nmcli connection show \"%s\" 2>/dev/null",
		...
		if (strstr(line, "ipv4.method:")) {
					...
					if (slash) {
						*slash = '\0';
						...
						if (prefix > 0 && prefix <= 32) {
							...
							snprintf(config->netmask, sizeof(config->netmask),
							         "%u.%u.%u.%u", (mask >> 24) & 0xFF,
							         (mask >> 16) & 0xFF, (mask >> 8) & 0xFF,
							         mask & 0xFF);
						}
					}
				}
			...
			if (colon) {
				...
				if (sscanf(colon, "%255[^\n]", dns_list) == 1) {
					...
					if (dns1) {
						snprintf(config->dns1, sizeof(config->dns1), "%s",
						         dns1);
						char *dns2 = strtok(NULL, " ,");
						if (dns2) {
							snprintf(config->dns2, sizeof(config->dns2), "%s",
							         dns2);
						}
					}
				}
...
int ethernet_set_dhcp(const char *device, const char *connection_name)
{
	...
	char cmd[512];
	snprintf(cmd, sizeof(cmd),
	         "nmcli connection modify \"%s\" ipv4.method auto 2>/dev/null",
	         conn_name);
	...
	snprintf(cmd, sizeof(cmd), "nmcli connection up \"%s\" 2>/dev/null",
	         conn_name);
	result = system(cmd);
	return (result == 0) ? 0 : -1;
}
                        ...
                        const char *gateway, const char *dns1, const char *dns2)
{
	...
	if (dns1 != NULL && strlen(dns1) > 0) {
		...
		if (dns2 != NULL && strlen(dns2) > 0) {
			snprintf(dns_str + strlen(dns_str),
			         sizeof(dns_str) - strlen(dns_str), " %s", dns2);
		}
	}
	...
	if (gateway != NULL && strlen(gateway) > 0) {
		if (strlen(dns_str) > 0) {
			snprintf(
			    cmd, sizeof(cmd),
			    "nmcli connection modify \"%s\" ipv4.method manual "
			    "ipv4.addresses %s ipv4.gateway %s ipv4.dns \"%s\" 2>/dev/null",
			    conn_name, cidr, gateway, dns_str);
		} else {
			snprintf(cmd, sizeof(cmd),
			         "nmcli connection modify \"%s\" ipv4.method manual "
			         "ipv4.addresses %s ipv4.gateway %s 2>/dev/null",
		...
		if (strlen(dns_str) > 0) {
			snprintf(cmd, sizeof(cmd),
			         "nmcli connection modify \"%s\" ipv4.method manual "
			         "ipv4.addresses %s ipv4.dns \"%s\" 2>/dev/null",
			         conn_name, cidr, dns_str);
		} else {
			snprintf(cmd, sizeof(cmd),
			         "nmcli connection modify \"%s\" ipv4.method manual "
			         "ipv4.addresses %s 2>/dev/null",
			         conn_name, cidr);
		}
	}
...
char *ethernet_config_to_json(const ethernet_config_t *config)
{
	...
	snprintf(json, size,
	         ...
	         "\"dns2\":\"%s\""
	         "}",
	         config->connection_name, config->device, config->method,
	         config->ip, config->netmask, config->gateway, config->dns1,
	         config->dns2);
