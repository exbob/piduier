...
int wifi_scan(wifi_network_list_t *list)
{
	...
	while (fgets(line, sizeof(line), fp) != NULL) {
		...
		if (count >= capacity) {
			capacity *= 2;
			wifi_network_t *new_networks =
			    realloc(list->networks, capacity * sizeof(wifi_network_t));
		...
		if (sscanf(line, "%7s %63s %31s %15s %d %31s %d %15s %31[^\n]", in_use,
		           net->ssid, net->bssid, mode, &net->channel, rate,
		           &net->signal, bars, net->security) >= 5) {
			net->in_use = (in_use[0] == '*');
			if (strlen(mode) > 0) {
...
int wifi_disconnect(const char *device)
{
	...
	if (device != NULL && strlen(device) > 0) {
		snprintf(cmd, sizeof(cmd), "nmcli device disconnect %s 2>/dev/null",
		         device);
	} else {
	...
	for (size_t i = 0; i < list->count; i++) {
		...
		int written =
		    snprintf(p, size - (p - json),
		             ...
		             "\"in_use\":%d"
		             "}",
		             (i > 0) ? "," : "", net->ssid, net->bssid, net->signal,
		             net->security, net->channel, net->mode, net->in_use);
